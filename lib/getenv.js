...
var objectFallbackEnabled = false;
var objectFallbackName;
...
/**
 * get the object fallback from the node environment
 */
function _fallback(fallback) {
  if (objectFallbackName === undefined) {
    return undefined;
  }

  return fallback[objectFallbackName];
}
...
function _value(varName, fallback) {
  ...
  if (value === undefined) {
    ...
    if (objectFallbackEnabled && isObject(fallback)) {
      var _default = _fallback(fallback);
...
getenv.enableObjectFallback = function (name) {
  objectFallbackEnabled = true;
  objectFallbackName = name;
};

getenv.disableObjectFallback = function () {
